...
var tagList = [];
var navBtnss = document.querySelectorAll('#nav-btns .slide-nav-btn');
var chbox = document.getElementById('checkbox-stop-slide');
...
for (var i = 0; i < navBtnss.length; i++) {
  tagList.push(navBtnss[i].dataset.id);
}

for (var i = 0; i < navBtnss.length; i++) {
  navBtnss[i].onclick = function () {
    var position = tagList.indexOf(this.dataset.id);
    position -= position != slidesToShow ? position * itemWidth : movePosition;
    track.style.cssText = 'transform: translateX(' + position + 'px)';
    sec();
    addAtribut();
    var b = document.querySelector(".slide-nav-btn.slide-nav-btn-active");
